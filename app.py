...
def createFirstGraph():
    ...
    tableFig = go.Figure(data=[
      go.Bar(name='Interactivity', x=interactivityDf['User'], y=interactivityDf['Interactivity'],text=interactivityDf["Interactivity"]),
      go.Bar(name='Percentage of Interactions', x=interactivityDf['User'], y=interactivityDf['Percentual'],text=interactivityDf["Percentual"])
    ])
...
def showMoreInteractivity():
    ...
    engagementFig = px.bar(dataframeFull, x="User", y="Engagement", color="User", title="User Engagement",text=dataframeFull["Engagement"])
    ...
    return render_template('interactivity.html', interactivity=dataframeFull.values.tolist(), fig=graphs,
                           ...
                           graphsImages=graphsImages, sizeFigs=len(graphs.keys()), keys=list(graphs.keys()),figTopSound=figTopSound, figTopUser=figTopUser)
@app.route('/queries')
def definedQueries():
...
def createTopGraphs():
    ...
    tableFig2 = px.bar(topUser, x="User", y="Participations", color="Participations", title="Top 5 Users",text=topUser["Participations"])
...
def querySoundChange():
    ...
    if(type == '2'):
        if(select == 'All Users'):
            ...
            graphs[0] = px.scatter(filter, x=filter.x, y=filter.y, color=filter.Sound, error_x_minus=None, error_y_minus=None, range_x=[-1000, 1000], range_y=[-1000, 1000]).to_html(full_html=False)
        else:
            filter = filter.astype({'Sound': 'str'})
            filter = filter.astype({'User': 'str'})
            fig = px.scatter(filter, x=filter.x, y=filter.y, color=filter.Sound, error_x_minus=None, error_y_minus=None, range_x=[-1000, 1000], range_y=[-1000, 1000])
    ...
    if (type == '3'):
        ...
        fig = px.scatter(query2, x=query2.x, y=query2.y, color=query2.User, error_x_minus=None, error_y_minus=None, range_x=[-1000, 1000], range_y=[-1000, 1000])
        graphs2[0] = fig.to_html(full_html=False)

    return render_template('respostas.html', interactivity=filter, data=data, data2=allSounds, selectValue=selectedValue,
...
def customQueries():
    ...
    return render_template("custom.html", query=[], inputValue='', inputValue2='', users=data, sounds=allSounds, user=None,
